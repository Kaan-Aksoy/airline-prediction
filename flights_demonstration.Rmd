...
```{r delays, include=TRUE, message=FALSE}
flights %>% 
...
```{r delays-per-airport, include=TRUE, message=FALSE}
flights %>% 
...
fit2 <- glm(delay ~ wind_speed + wind_gust + humid + precip + pressure + visib + temp,
            data = df1,
            family = "binomial")
